...
public class Stream_01_Test {
	...
	public void test_stream_filter() throws Exception {
		...
		List<Pizza> result = pizzas.stream().filter(prix -> prix.getPrice() >= 1300).toList();
	...
	public void test_stream_filter_collect_counting() throws Exception {
		...
		long result = pizzas.stream().filter(prix -> prix.getPrice() >= 1300).count();
	...
	public void test_stream_findFirst() throws Exception {
		...
		Optional<Order> result = orders.stream().filter(name->name.getCustomer().getFirstname().equals("Sophie")).findFirst();
	...
	public void test_stream_filter_adv() throws Exception {
		...
		// pizza Pépéroni
		List<Order> result = orders.stream().filter(order->order.getPizzas().stream().anyMatch(pizza->pizza.getName().equals("Pépéroni"))).toList();
