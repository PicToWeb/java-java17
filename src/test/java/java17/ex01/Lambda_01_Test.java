...
public class Lambda_01_Test {
    ...
    public void test_filter_by_age() throws Exception {
        ...
        List<Person> result =personList.stream().map(s->s).filter(m->m.getAge()>=18).toList();
    ...
    public void test_filter_by_firstname() throws Exception {
        ...
        List<Person> result = personList.stream().map(s->s).filter(m->m.getFirstname().equals("first_10")).toList();
    ...
    public void test_filter_by_password() throws Exception {
        ...
        List<Person> result = personList.stream().map(s->s).filter(m->m.getAge()>49 && DigestUtils.sha512Hex(m.getPassword()).equals(passwordSha512Hex)).toList();
